...
from hummingbot.connector.exchange.eqonex.eqonex_constants import EQONEX_WS_URI
...
class EqonexAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def logger(cls) -> HummingbotLogger:
        if cls._eqonexausds_logger is None:
            cls._eqonexausds_logger = logging.getLogger(__name__)

        return cls._eqonexausds_logger

    def __init__(self, eqonex_auth: EqonexAuth, trading_pairs: Optional[List[str]] = []):
        self._current_listen_key = None
        self._current_endpoint = None
        self._listen_for_user_steam_task = None
        self._last_recv_time: float = 0
        self._auth: EqonexAuth = eqonex_auth
    ...
    async def _authenticate_client(self):
        """
        Sends an Authentication request to Eqonex's WebSocket API Server
        """
        msg = json.dumps(self._auth.generate_ws_auth()).replace(" ", "")
        await self._websocket_connection.send(msg)
    ...
    async def get_ws_connection(self):
        stream_url: str = f"{EQONEX_WS_URI}"
